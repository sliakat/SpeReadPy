...
numFrames = 20
targetMem = 512 #target MiB of RAM to allocate for circular buffer
...
def analyzeDat(dat,data,frameCt,stride,rate):
    global count
    if rate==0:
        rate=0.01
    ...
    if np.mod(count, np.ceil(rate))==0 or count==1 or count==frameCt:    #update display approx every second, and first annd last frame
        if rowVal.value==1:
            ax.clear()
            ax.plot(data)
        else:
            display_min = np.int(np.percentile(data,5))
            display_max = np.int(np.percentile(data,95))
            data = np.reshape(data,[rowVal.value,colVal.value])
            if display_min < 1:
                display_min = 1       
            
            ax.imshow(data,vmin=display_min,vmax=display_max,cmap='gray')
        fig.show()
        plt.pause(.001)
        print("Frames Acquired: %d, Mean Pixel Value of Most Recent Readout: %5.2f."% (count,np.mean(data)))
...
if errOpenCam == 0:    
    ...
    #set up the plot
    dataI = np.zeros(colVal.value*rowVal.value,dtype=np.uint16)
    fig = plt.figure(1)
    ax = fig.add_subplot(111)
    if rowVal.value>1:        
        reshI = np.reshape(dataI,[rowVal.value,colVal.value])
        pltObj = ax.imshow(reshI,cmap='gray')        
    else:
        pltObj = ax.plot()
    fig.show()
    plt.pause(.001)
    ...
    if errCom==0:        
        ...
        analyzeDat(dat,dataI,frameCount.value,readStride.value,readRate.value)
